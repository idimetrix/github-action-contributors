...
type CollaboratorsUserData =
  Endpoints["GET /repos/{owner}/{repo}/collaborators"]["response"]["data"][number];
...
class Generator {
  ...
  async getContributors() {
    ...
    startGroup(
      `Contributors UserInfo: (${list?.length}) \x1b[34m(GET /repos/${this.owner}/${this.repo}/contributors)\x1b[0m`,
    );
    ...
    if (list && list.length > 0) {
      ...
      list
        ...
        .forEach((item) => this.dataBot.push(item));
      if (this.options.filterAuthor) {
        userData = list.filter(
          (item) => !new RegExp(this.options.filterAuthor).test(item.login),
        );
      }
     ...
     */
    const collaboratorsList = await octokit.paginate(
    ...
    startGroup(
      `Collaborators UserInfo: (${collaboratorsList?.length}) \x1b[34m(GET /repos/${this.owner}/${this.repo}/collaborators)\x1b[0m`,
    );
    ...
    if (collaboratorsList && collaboratorsList.length > 0) {
      ...
      collaboratorsList
        .filter((el) => el.type === "Bot" || el.login.includes("actions-user"))
        .forEach((item) => this.dataBotCollaborators.push(item));
      if (this.options.filterAuthor) {
        userData = collaboratorsList.filter(
          (item) => !new RegExp(this.options.filterAuthor).test(item.login),
        );
      }
      if (this.options.excludeBots) {
...
<image x="${x}" y="${y}" width="${width}" height="${height}" xlink:href="${img}"/>
    ...
    const contributorsHeight = calcSectionHeight(
      this.data.length,
      this.options,
    );
    this.svg = this.options.svgTemplate
      .replace("{{ width }}", String(this.options.svgWidth))
  ...
  async outputMarkdown() {
    ...
    /** Contributors Bots */
    const {
      htmlList: htmlListBots,
      htmlTable: htmlTableBots,
      colCount: colCountBots,
    } = await this.getHTMLStr(this.dataBot);
    startGroup(
      `Contributors Bots: \x1b[34m(htmlListBots)\x1b[0m ${colCountBots}`,
    );
    info(`${htmlListBots}`);
    endGroup();
    ...
    /** Collaborators */
    const {
      htmlList: htmlCollaboratorsList,
      htmlTable: htmlCollaboratorsTable,
      colCount: collaboratorsColCount,
    } = await this.getHTMLStr(this.dataCollaborators);
    startGroup(
      `Collaborators : \x1b[34m(htmlCollaboratorsTable)\x1b[0m ${collaboratorsColCount}`,
    );
    ...
    /** Collaborators Bots */
    const {
      htmlList: htmlCollaboratorsListBots,
      htmlTable: htmlCollaboratorsTableBots,
      colCount: collaboratorsColCountBots,
    } = await this.getHTMLStr(this.dataBotCollaborators);
    startGroup(
      `Collaborators Bots: \x1b[34m(htmlCollaboratorsListBots)\x1b[0m ${collaboratorsColCountBots}`,
    );
    ...
    startGroup(
      `Collaborators Bots: \x1b[34m(htmlCollaboratorsTableBots)\x1b[0m`,
    );
